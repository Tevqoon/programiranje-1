...
let rec intbool_map f_int f_bool ib_list = match ib_list with 
       | Empty -> Empty
       | Int(x, rest) -> Int(f_int(x), intbool_map f_int f_bool rest)
       | Bool(x, rest) -> Bool(f_bool(x), intbool_map f_int f_bool rest)
...
let intbool_reverse ib_list = 
       let rec r finished todo = match todo with
              | Empty -> finished
              | Int(x, rest) -> r (Int(x, finished)) rest
              | Bool(x, rest) -> r (Bool(x, finished)) rest
       in r Empty ib_list
...
let rec intbool_separate ib_list = 
       let rec r inti booli todo = match todo with
              | Empty -> (inti, booli) 
              | Int(x, rest) -> r (x :: inti) booli rest
              | Bool(x, rest) -> r inti (x :: booli) rest
       in r [] [] ib_list
...
let update ({arcane = vrednost_arcane} as magic_counter) magic_type = match magic_type with
       ...
       | Arcane -> {magic_counter with arcane = vrednost_arcane + 1}
...
let count_magic l =
       let rec r todo counter = match todo with
              | [] -> counter
              | x :: xs when x.status = Newbie -> r xs counter
              | x :: xs when x.status = Student -> r xs (update counter (fst x.status))
